...
module Make (Handlers:OXMODULE) = struct
  ...
  let connect_controller (host: string) (port : int) =
    (* What we need is here catch the Disconnect event and try to reconnect *)
    (* Whenever we connect  *)
    Platform.connect_to_controller host port
  ...
  let start_hypervisor () : unit Lwt.t = 
    Platform.init_with_port 6633 >>
    Lwt.pick [ handle_deferred (); 
      connect_controller "127.0.0.1" 7000;
      connect_controller "127.0.0.1" 7001;
      accept_switches () ]

  let _ =
    (* intentionally on stdout *)
    Format.printf "Ox hypervisor launching...\n%!";
    Sys.catch_break true;
    try
      Lwt_main.run (start_hypervisor ())
    with exn ->
      Handlers.cleanup ();
